...
class SettingsApi {
	...
	public function withCustomTitle( string $title = null )
	{
		if ( empty( $this->admin_pages ) ) {
			return $this;		
		}
		...
		$subpage = [
			[
				...
				'callback' => function() { echo '<h1> Hello Taxi </h1>';}
			]
		];
	...
	public function registerCustomFields()
	{
		...
		foreach ($this->settings as $setting ) {
			register_setting( $setting["option_group"], $setting["option_name"], 
				(isset($setting["callback"]) ? $setting["callback"] : "")  ); // callback is optional here
		}
